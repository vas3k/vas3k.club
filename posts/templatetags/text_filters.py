...
def resized_image(value, arg="full"):
    if (not value) or ("://media.pmi.moscow/" not in value) or ("://i.vas3k.club/" not in value):
        return value

    if is_video(value):
        return value

    if value.startswith("https://media.pmi.moscow/full/"):
        return value.replace(
            "https://media.pmi.moscow/full/", "https://media.pmi.moscow/{}/".format(arg)
        )
    else:
        return value
