...
from comments.forms import BattleCommentForm, CommentForm, ReplyForm
...
def render_post(request, post, context=None):
    ...
    if request.me:
        comments = (
            Comment.objects_for_user(request.me).filter(post=post).all()
        )  # do not add more joins here! it slows down a lot!
        ...
        upvoted_at = (
            int(
                PostVote.objects.filter(post=post, user=request.me)
                .first()
        ...
        muted_user_ids = list(
            UserMuted.objects.filter(user_from=request.me)
            .values_list("user_to_id", flat=True)
            .all()
        )
        user_notes = dict(
            UserNote.objects.filter(user_from=request.me)
            .values_list("user_to", "text")
            .all()[:100]
        )
        collectible_tag = (
            Tag.objects.filter(code=post.collectible_tag_code).first()
            if post.collectible_tag_code
            else None
        )
        is_collectible_tag_collected = (
            UserTag.objects.filter(tag=collectible_tag, user=request.me).exists()
            if collectible_tag
            else False
        )
    else:
    ...
    if comment_order in POSSIBLE_COMMENT_ORDERS:
        ...
        )  # additionally sort by time to preserve an order
    ...
    comment_form = (
        CommentForm(initial={"text": post.comment_template})
        if post.comment_template
        else CommentForm()
    )
    context = {
        **(context or {}),
