...
class CoffeesDay(AnticBase):
    ...
    notifications = [
        "_{sender} решил напомнить, что сегодня день кофе, и надо его пить! ☕",
        "_В честь дня кофе {sender} подкинул немного в чат. Разбирайте:_\n\n☕☕☕☕☕☕",
        "_Сегодня день кофе, и {sender} напоминает: выпей ещё чашечку._",
        "_Нужно раздуплиться? Как вовремя, что сегодня день кофе, и {sender} советует выпить чашечку._",
        "_Чат, внимание: в день кофе вам нельзя прекращать его пить, и как раз {sender}"
        " принёс новую чашку_ ☕",
        "_Сегодня {sender} крайне бодр. Настолько, что в честь Дня кофе принёс пару чашек сюда ☕☕_",
        "_По всяким интернет-версиям, кофе спасает от головоболезней. {sender} им, конечно,"
...
class WesternChristmasPrivate(AnticBase):
    ...
    notifications = [
        ...
        "{sender} поделился с тобой рождественским вайбом, так что теперь вайбуй! ✨",
        "Лови снежок! ☃️\nЭто {sender} решил таким образом поздравить тебя с рождеством.",
        "🎁\nЧто это? Подарок? Открывай!\nТочно, это {sender} передал поздравления с Рождеством!",
