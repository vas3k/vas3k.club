ACHIEVEMENTS = [
    ("old", {
        "name": "Олды здесь",
        "description": "Заслуженный ранний член Клуба",
        "image": "https://i.vas3k.club/3bl.png",
        "style": "color: #FFF; background-color: #65c3ba;",
    }),
    ("investor_1000", {
        "name": "Ёрли Инвестор",
        "description": "Человек, вложивший $1000 и более реальных денег в развитие Клуба",
        "image": "https://i.vas3k.club/3a1.png",
        "style": "background-color: #FFDC00;",
    }),
    ("contributor", {
        "name": "Контри-бьютор",
        "description": "За значительный вклад в разработку Клуба",
        "image": "https://i.vas3k.club/3ah.png",
        "style": "background-color: #205072;",
    }),
    ("team", {
        "name": "Команда Клуба",
        "description": "Входит в команду модераторов, кураторов контента или других людей, "
                       "следящих чтобы Клуб жил и процветал",
        "image": "https://i.vas3k.club/3bb.png",
        "style": "background-color: #FE676E;",
    }),
    ("oscar", {
        "name": "Оскар за посты",
        "description": "Награда за написание особенно офигенных постов, ни один раз занимавших первые места в топах",
        "image": "https://i.vas3k.club/3bd.png",
        "style": "background-color: #2F9599;",
    }),
    ("miner", {
        "name": "Добытчик Золота",
        "description": "Нашел и запостил много офигенных ссылок, которые ни раз попадали в топы недели",
        "image": "https://i.vas3k.club/3d2.png",
        "style": "background-color: #F8D90F;",
    }),
    ("helper", {
        "name": "Твой Бро",
        "description": "Пятюня за активную помощь другим участника Клуба в комментариях и в жизни",
        "image": "https://i.vas3k.club/3bo.png",
        "style": "background-color: #4D446F;",
    }),
    ("offline", {
        "name": "За взятие офлайна",
        "description": "Медаль за участие в офлайн-сходках. "
                       "Считается любое собрание, на котором присутствовало 5 и более членов Клуба. "
                       "Выдаётся при наличии фото-подтверждений",
        "image": "https://i.vas3k.club/3ai.png",
        "style": "background-color: #94B447;",
    }),
    ("international", {
        "name": "Между-народные отношения",
        "description": "Выдаётся за офлайн-встречу с ранее не знакомыми членами Клуба, проживающими в другой стране. "
                       "Нужны фото-доказательства",
        "image": "https://i.vas3k.club/3aq.png",
        "style": "background-color: #1976D2; font-size: 120%;",
    }),
    ("cook", {
        "name": "Звезда Мишлен",
        "description": "Активный участник кулинарных постов",
        "image": "https://i.vas3k.club/3c8.png",
        "style": "background-color: #E9765B;",
    }),
    ("tractor", {
        "name": "Почётный Тракторист",
        "description": "Завсегдатай трактор-тредов",
        "image": "https://i.vas3k.club/3c9.png",
        "style": "background-color: #CCABD8; font-size: 120%;",
    }),
    ("travel", {
        "name": "Гражданин Мира",
        "description": "Знает кучу лайфхаков о путешествиях",
        "image": "https://i.vas3k.club/3ca.png",
        "style": "background-color: #EA4492; font-size: 130%;",
    }),
    ("altruist", {
        "name": "Альтруист",
        "description": "Дал Клубу больше, чем вообще возможно. "
                       "Выдаётся за активность в разделе «Кумовство» и другую помощь коммьюнити",
        "image": "https://i.vas3k.club/3d0.png",
        "style": "background-color: #428CD4;",
    }),
    ("ideas", {
        "name": "Идейный Вдохновитель",
        "description": "Запостил хороших постов в раздел «Идеи»",
        "image": "https://i.vas3k.club/3d1.png",
        "style": "background-color: #F04393; font-size: 100%;",
    }),
]


# VIP: https://i.vas3k.club/3cb.png
